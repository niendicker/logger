...
{
	slave* mbDevice = loadDeviceConfiguration("modbus.conf");
   assert(mbDevice != NULL);
   ...
   char* host_name = strtok(mbDevice->link.modbusTcp.hostname, "\n");
   ...
   server.sin_addr.s_addr = inet_addr(host_ip_addr);
	server.sin_port = htons( mbDevice->link.modbusTcp.port );
